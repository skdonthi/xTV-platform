...
export function createXtvAliases(workspaceRoot: string, customer = "ccl"): Record<string, string> {
  return {
    ...
    // Fonts + themes are build-time brand assets (like release.json), NOT head-end
    // config. Per-tenant so a brand's palette never ships in another brand's build.
    "@x-tv/tenant/fonts": resolve(workspaceRoot, `customers/${customer}/fonts.json`),
    "@x-tv/tenant/themes": resolve(workspaceRoot, `customers/${customer}/themes.json`),
